...
2. **Parsing** — the raw pattern text is sent to Gemini, which extracts structured data: the pattern's gauge, and each section (ribbing, front panel, sleeve decreases, etc.) with its stitch count, row count, and repeat constraints. If the pattern lists multiple sizes, a preferred size label can be passed through so Gemini extracts counts for that size specifically (falling back to the middle/nearest size if the label isn't found).
...
- **Only one periodic shaping cadence per section is rescaled.** Beyond a section's flat endpoint stitch/row counts, `rescale.ts`'s `rescaleShaping()` also recomputes a section's *periodic* shaping rate — how many times a decrease/increase round is worked (`shaping_event_count`), and the row/round interval between each one (`shaping_interval_rows`) — so a knitter following the rescaled "every Nth round, M times" text lands on the same rescaled endpoint counts the pattern's own text shows. This is a best-effort approximation, not exact: the recomputed event count and interval are derived from the section's already-independently-rounded start/end stitch counts and row count, so they won't always multiply back to precisely those totals. A section with more than one distinct shaping phase (e.g. a sock toe that decreases at one rate then a faster rate near the end) only has its single most significant cadence captured — the others aren't adjusted and may need manual tweaking.
...
- **LLM parsing isn't perfectly deterministic.** Each run sends the pattern text to Gemini fresh, so two separate runs can occasionally return slightly different section boundaries or values — worse on patterns with many repeated, similarly-shaped size lists or checkpoint sentences (e.g. "You should still have N sts on your needles" restated many times throughout a pattern). Temperature is set to 0, `max_tokens` is generous, and the prompt requires a verbatim source quote per section plus explicit rules for section boundaries and size-position resolution, which together substantially reduce (but don't eliminate) this. Some pattern structures also don't map cleanly onto the extraction schema at all — a section spanning several rounds that each restate their own running stitch total has no single canonical "the" count for that section. Switching from Claude to Gemini (for the free tier) measurably regressed this. The app ships on `gemini-3.5-flash-lite`, which can drop whole sections outright (not just disagree on a value) across otherwise-identical calls at temperature 0 — worse than Claude Haiku ever was, even after porting over the same checkpoint-sentence and section-boundary rules that worked for Claude. A bigger, more consistent Gemini model was tested and rejected: it spends a large, variable token budget on hidden reasoning before the visible output (so `maxOutputTokens` needs real headroom above that, or responses truncate mid-JSON), and more importantly its free tier caps at 20 requests/day — unworkable for a deployed app with real traffic, versus flash-lite's much more generous per-minute-based free limit. Closing this consistency gap on flash-lite would take dedicated prompt-tuning investment similar to what the original Claude non-determinism work took; not pursued further as of 2026-07-27. Use `npm run test:patterns -- --repeat N` (see below) to check whether a specific pattern parses consistently before trusting a one-off result.

## Stack

- **Backend**: Node.js + TypeScript, Express, `pdfjs-dist` (extraction), `pdf-lib` (stamping), Google Gen AI SDK (`@google/genai`) with Zod structured outputs for parsing pattern text.
- **Frontend**: React + TypeScript (Vite).

## Running locally

You'll need a `GEMINI_API_KEY` set in your environment.
...
The frontend (static) and backend (needs a running Node process + your Gemini API key kept secret) are hosted separately, both on free tiers.
...
- `promptingGemini.ts` — sends pattern text to Gemini and returns structured gauge/section data.
...
```
Runs one or more specific patterns (matched by exact or partial filename) with your own gauge/size/offset, `--repeat N` times each. Reads `test.config.json` at the repo root by default (edit it directly instead of typing flags every time — see below), or point at a different file with `--config`; any CLI flag passed overrides the config for that run. When `repeat` is greater than 1, it also diffs each section's original/rescaled numbers across runs and flags any that differ — since `prompting()` hits Gemini fresh every call (see the non-determinism note above), this is how to check whether a given pattern parses consistently before trusting a one-off result.

`test.config.json` is gitignored (so your own local tweaks — testing one file at a time, adjusting repeat counts, etc. — never show up as diffs). Copy `test.config.example.json` to `test.config.json` to get started; the example file tracks the full sample-pattern set and is what should get updated if the actual defaults need to change.
